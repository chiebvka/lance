...
export default function ReceiptConfirmModal({ 
  ...
  const canSend = ["sent", "overdue"].includes(receiptState)

  const handleSendReminder = async () => {
    setIsLoading(true)
    try {
      const res = await fetch('/api/receipts/send-reminder', {
        method: 'POST',
            ...
            <div>
              ...
              <DialogDescription className="text-left mt-1">
                {canSend
                  ...
                  : <>Reminders can only be sent for receipts in <b>sent</b> or <b>overdue</b> state.</>
                }
              </DialogDescription>
            </div>
          </div>
