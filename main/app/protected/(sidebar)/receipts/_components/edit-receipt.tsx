...
import { useReceipt, useUpdateReceipt } from '@/hooks/receipts/use-receipts';
import { CreateReceiptData } from '@/hooks/receipts/use-receipts';
...
export interface EditReceiptRef {
...
const EditReceipt = forwardRef<EditReceiptRef, Props>(({ 
  ...
  useEffect(() => {
    if (receipt && !receiptLoading) {
      // Set dates
      if (receipt.issueDate) setIssueDate(new Date(receipt.issueDate))
      ...
      // Set layout options
      setLayoutOptions({
        hasTax: receipt.hasTax || false,
        hasVat: receipt.hasVat || false,
        hasDiscount: receipt.hasDiscount || false
      })
      
      // Set rates
      setTaxRate(receipt.taxRate || 10)
      setVatRate(receipt.vatRate || 20)
      setDiscountAmount(receipt.discount || 0)
      
      // Set currency
      const receiptCurrency = currencies.find(c => c.code === receipt.currency)
      if (receiptCurrency) {
        setSelectedCurrency(receiptCurrency)
      }
      
      // Set receipt items
      if (receipt.receiptDetails && Array.isArray(receipt.receiptDetails)) {
        const items = receipt.receiptDetails.map((item: any, index: number) => ({
          id: index + 1,
        ...
        setReceiptItems(items.length > 0 ? items : [
          { id: 1, description: '', quantity: 0, price: 0, total: 0, position: 0 }
        ])
      }
    }
  }, [receipt, receiptLoading, userEmail, organization])
    ...
    const getReceiptData = (): Partial<CreateReceiptData> => {
        return {
  ...
  const handleSubmit = async (emailToCustomer = false) => {
    ...
    if (emailToCustomer && !selectedCustomer) {
      toast.error("Please select a customer to send the receipt")
      return
    }
    ...
    try {
      setLoading(true)
      await updateReceiptMutation.mutateAsync({ receiptId, receiptData })
      
      toast.success(emailToCustomer ? "Receipt updated and sent successfully!" : "Receipt updated successfully!")
      onSuccess?.()
    } catch (error: any) {
  ...
  if (receiptError || (!receipt && !receiptLoading)) {
    return (
      <div className="p-6 text-center">
        <p className="text-red-500">
          Error loading receipt: {receiptError?.message || 'Please try again.'}
        </p>
    ...
    const canEdit = receipt?.state && ["draft", "unassigned", "sent", "settled", "overdue"].includes(receipt.state)
    const canDelete = receipt?.state && ["draft", "unassigned"].includes(receipt.state)
    const canSendEmail = receipt?.state && ["draft", "unassigned", "sent", "overdue"].includes(receipt.state)
  
    if (!canEdit) {
      return (
        <div className="p-6 text-center">
          <p className="text-muted-foreground mb-4">
            This receipt cannot be edited because it is {receipt?.state}.
          </p>
    ...
    return(
        ...
        <div className='max-w-4xl mx-auto'>
            ...
            <div className='flex-1'>
              <div className='space-y-2 text-sm '>
                <div className='flex items-center gap-4'>
                  ...
                  <span className=''>{receipt?.receiptNumber || 'Loading...'}</span>
                </div>
                <div className='flex items-center gap-4'>
          ...
          {/* Inline toggles for layout options while editing */}
          {/* <div className='mb-4 flex flex-wrap gap-4'>
            <div className='flex items-center gap-2'>
              <Label className='text-sm'>Add sales tax:</Label>
              <ComboBox
                items={[{value:'yes',label:'Yes'},{value:'no',label:'No'}]}
                value={layoutOptions.hasTax ? 'yes' : 'no'}
                onValueChange={(v)=> setLayoutOptions(prev=>({...prev, hasTax: v==='yes'}))}
                placeholder=""
              />
            </div>
            <div className='flex items-center gap-2'>
              <Label className='text-sm'>Add VAT:</Label>
              <ComboBox
                items={[{value:'yes',label:'Yes'},{value:'no',label:'No'}]}
                value={layoutOptions.hasVat ? 'yes' : 'no'}
                onValueChange={(v)=> setLayoutOptions(prev=>({...prev, hasVat: v==='yes'}))}
                placeholder=""
              />
            </div>
            <div className='flex items-center gap-2'>
              ...
              <ComboBox
                items={[{value:'yes',label:'Yes'},{value:'no',label:'No'}]}
                value={layoutOptions.hasDiscount ? 'yes' : 'no'}
                onValueChange={(v)=> setLayoutOptions(prev=>({...prev, hasDiscount: v==='yes'}))}
                placeholder=""
              />
            </div>
          </div> */}
...
EditReceipt.displayName = 'EditReceipt'
